...
public class AdminController {
	...
	public String board_view(@RequestParam("bno")Integer bno,@ModelAttribute("pageVO")PageVO pageVO, Model model) throws Exception {
		BoardVO boardVO = boardService.readBoaard(bno);
		...
		//개발자가 만든 클래스형 객체 boardvo는 개발자가 만듵 메서드 사용
		...
		//위 for은 세로데이터(다수레코드)를 가로데이터(1레코드이면, 배열)에 담아서 1개 레코드 boardVO로 만드는게 목적.
		...
		model.addAttribute("boardVO", boardVO);//게시물 + 처부파일 명2개
		//업로드한 파일이 이미지인지 아닌지 확인하는 용도의 데이터 입니다.(목적: 이미지일때 미리보기 img태그를 사용하기위해서)
		model.addAttribute("checkImgArray", commonUtil.getCheckImgArray());
		return "admin/board/board_view";//.jsp생략
	}
	
	//게시물 목록은 폼으로 접근하지 않고 URL로 접근하기 때문에
