...
/**
 * 이 클래스는 Admin관리자단을 접근하는 클래스 <- 디스패치 서블렛(게이트웨이) 기능을 합니다.
 * 디스패처 서블렛 클래스는 톰캣이 실행될때 제일 먼저 실행되는 클래스, 그래서, 게이트웨이라고 합니다.
...
public class AdminController {
	...
	//왜 board_type하지않고, bbs_type하는 이유는 왼쪽메뉴 고정시키는 로직에서 경로 board와 겹치지 않도록 하기위해
	...
	public String selectBoardTypeList() throws Exception {//목록폼1
		return null;//상대경로일떄 view폴더가 root(최상의)
	}
	...
	public String updateBoardTypeForm1() throws Exception {//수정폼1
		return null;
	}
	@RequestMapping(value="admin/bbs_type/bbs_type_update", method=RequestMethod.POST)
	public String updateBoardTypeForm() throws Exception {//수정처리1
		return null;
	}	
	...
	//===================
	//아래경로는 회원신규등록 폼을 호출하는 URL쿼리스트링으로 보낸것을 받을때는 GET방식으로 받습니다.
