...
public class AspectAdvice {
	...
	//나중에 게시물관리 기능 만들때 @Aspect로 AOP기능 추가 = 목적: board_type값을 항상 가져가 가도록 처리(세션)
	//세션? 서버-PC 구조상에서 클라이언트가 서버에 접속할때 [서버에 발생되는 정보를 세션이라고 함(서버에 저장됨)]
	//쿠키? 서버-PC 구조상에서 클라이언트가 서버에 접속할떄 클라이언트에 발생되는 정보를 쿠키라고함(PC에 저장됨)
	//옛날에는 쿠키로 로그인 체크를 했음. ->보안상 PC에 로그인정보가 저장되기 때문에 위험(인터넷광고에만 사용) -> 세션만 사용	
	//Aspect로 AOP를 구현할떄는 포인트컷(Advice참견이 실행될 위치)이 필요합니다.
	//@Around=@Before+@After = @Around(포인트컷 전+후.*(...)모든 메서드)
	//@Around는 콜백함수 매개변수로 조인포인트객체(포인트컷에서 입출력값)를 필수로 받습니다.
	...
	public Object sessionManager(ProceedingJoinPoint pjp) throws Throwable {
		...
		//조인포인트리스트의 객체들의 메서드의 Arguments매개변수를 뽑아냄
		for(Object object:pjp.getArgs()) {
			if(object instanceof PageVO) {//AOP실행메서드중 매개변수 PageVO pageVO객체 판단
				//결과는 게시판, 맴버 서비스에 PageVO사용하는 서비스는 적용됨. 게시판생성관리에는 적용X
		...
		if(request != null) {//jsp에서 Get,Post 있을때,
			...
			HttpSession session = request.getSession();//PC가 스프링프로젝츠 접근시 세션객체 
			...
			if(session.getAttribute("session_board_type") != null) {
				...
				if(pageVO != null) {//Set은 pageVO null아닐 경우만 실행되도록
	...
	//이 메서드는 컨트롤러에서 Exception이 발생했을때 여기서 인터셈터(가로채기)해서
	...
	public ModelAndView errorModelAndView(Exception ex, HttpServletRequest request) {
		...
		request.getSession().setAttribute("prevPage", referer);//prevPage세션변수사용
		//--------------------------------------------
		//컨트롤러에서 받은 Exception을 ModelAndCiew로 전달(아래)
		modelAndView.addObject("exception", ex);
		modelAndView.setViewName("home/error_spring");//return String .jsp생략
		return modelAndView;
	}
	//이 메서드는 컨트롤러로 메서드가 실행 전에 값을 생성해서 model객체에 담아서 jsp로 자료를 전송
