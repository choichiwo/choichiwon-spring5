...
--select: 테이블내용 조회, where 조회조건 as(Alies)별칭으로 필드명이 길때
...
-- Mysql(마리아DB) 있는 AI(AutoIcrement)자동증가값 명령 오라클X
...
-- 함수용어 ABS(Absolute절대값), Floor(바닥함수1.5=1)<->cell(천장함수1.5=2)
-- ROUND(반올림), TRUNC(Truncate버리는합수), MOD(나머지구하는함수)
