...
#### 20210607(월) 작업.
- 오라클 DB관리 로그인정보(Application Express웹프로그램이름): admin/apmsetup -> 암호변경요청: admin/Apmsetup1234!(대문자추가+특수문자추가)
```
...
- 만약 시퀸스(AI)기능이 없다면, 게시물 작성시 첫번째, 두번쨰 게시물 등등을 구별하는
- 숫자를 계속 추가하려면, 현재까지 저장된 게시물의 최고번호값(Max)을 구해ㅓ +1 해야 합니다.(개발자가 Insert시)
- DB에서 기본으로 위 Max값을 구해서 +1하는 로직을 만들게 됩니다. 이 기능이 시퀸스(AI) 입니다.
- 우리 스프링 프로젝트에서는 2개 시퀸스를 만듭니다.(게시물 시퀸스, 댓글 시퀸스)
...
- 프로젝트의 버전을 올립니다. -> 외부라이브러리 부분의 버전을 올림 - 메이븐 -> pom.xml 버전을 관리 메이븐은 수정 및 추가 -> maven 메뉴에서 maven 
- controller클래스에서 생성한 변수 사용(자바)model객체전송(스프링)return home 
- ->(전송은스프링) home.jsp 출력
- 출력하는 방식1: 25년전부터 방식 EL(Express Language)방식출력-${변수} , <%=${변수} %>
- 많이사용하는 방식 JSTL방식(*표준) 사용,반복,조건 등등 다양한 방식
...
- 1.@Controller 클래스빈: 라우터역할 메서드가 만듭니다.(라우터역활의클래스기능)
- 2.@Service 클래스빈: 비지니스로직 메서드가 만듭니다.(개발자위주의 기능)
...
- html를 분해(1개의 페이지를 2개로 분해, 1.header.jsp(메뉴를공통), 2.footer.jsp(공통))
